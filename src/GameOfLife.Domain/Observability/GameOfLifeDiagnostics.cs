...
public static class GameOfLifeDiagnostics
{
    ...
    public static readonly Counter<long> GenerationsComputed =
        Meter.CreateCounter<long>("gameoflife.generations_computed", "{generation}",
            "Generations computed across all requests.");

    public static readonly Histogram<double> EvolutionDurationMs =
        Meter.CreateHistogram<double>("gameoflife.evolution_duration", "ms",
            "Wall-clock time spent computing generations for a single request.");

    public static readonly Counter<long> ConvergenceOutcomes =
        Meter.CreateCounter<long>("gameoflife.convergence_outcomes",
            description: "Final-state requests, tagged by whether a cycle was found within budget.");
}