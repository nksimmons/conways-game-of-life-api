...
{
    private const string ErrorType = "https://gameoflife.example/problems/invalid-request";
    private const string ErrorTitle = "The request is invalid.";

    private const string UnreadableValue = "The value supplied is not valid for this field.";

    public static ValidationProblemDetails ToProblemDetails(this IDictionary<string, string[]> errors) =>
        new(CamelCaseKeys(errors))
        {
            Type = ErrorType,
            Title = ErrorTitle,
    ...
    private static string Describe(ModelError error) =>
        error.Exception is not null || string.IsNullOrWhiteSpace(error.ErrorMessage) ||
        IsBinderText(error.ErrorMessage)
            ? UnreadableValue
            : error.ErrorMessage;
    ...
    private static Dictionary<string, string[]> CamelCaseKeys(IDictionary<string, string[]> errors) => errors.ToDictionary(pair => ToCamelCase(pair.Key), pair => pair.Value, StringComparer.Ordinal);
    ...
    private static string ToCamelCase(string key) => string.IsNullOrEmpty(key) ? key : JsonNamingPolicy.CamelCase.ConvertName(key);
}
